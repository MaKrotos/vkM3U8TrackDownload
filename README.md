...
namespace vkPosterBot.TgBot
{
    ...
    public class M3U8Downloader
    {
        ...
        public M3U8Downloader(string m3u8Url)
        {
            _m3u8Url = m3u8Url;
        }

        public void DownloadAndCombineAudio(string outputPath)
        {
            var m3u8FileContent = DownloadM3U8File();
            var audioUrls = ParseM3U8File(m3u8FileContent);
            CombineAudioFiles(audioUrls);
        }
        ...
        Dictionary<string, byte[]> strKeys = new();
        Dictionary<string, string> strKeysz = new();
        ...
        private void CombineAudioFiles(List<AudioSegment> audioSegments)
        {
            string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "segments");
            byte[] buffer = new byte[1024];

            using (WaveFileWriter waveFileWriter = new WaveFileWriter(path + "\\qqww.mp3", new WaveFormat()))
            {
